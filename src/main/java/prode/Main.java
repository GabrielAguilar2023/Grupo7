...
//----------Creacion de todos los Objetos necesarios para el proceso
            ...
            if (infoDebug.equals("SI")) for (Ronda ronda : rondas) ronda.infoRonda();
            //Procesamiento de los datos pronosticos
            if (!mySql_OK) {
...
// Crear vector con todos los Equipos del campeonato sin que se repitan para poder hacer seguimiento de los pronosticos por equipos
        ...
        for (int j = 0; j < partidos.length; j++) {  // Carga el HashSet con los equipos desde todos los partidos
...
// Buscar en que partido jugo cada equipo
        ...
        for (int i = 0; i < vectorDeEquipos.length; i++) {
            boolean jugoPartidoEnEstaFase=false; // Pregunto para saber si creo vector con informacion de partidos por fase para este equipo
            ArrayList <String> partidosEnLaFase = new ArrayList<>(); //Lista de partidos jugados por el equipo en cuestion
